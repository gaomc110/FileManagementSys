...
layui.define(['jquery', 'form', 'layer'], function(exports) {
    ...
    ajax.prototype.init = function() {
        ...
        /*$(".ajax:not([ajax-bind])").each(function() {
        	alert(1);
            _this.bind($(this));
            $(document).on('click',{ ajax: _this }, _this.click);
        });*/
        $(document).on('click','.ajax:not([ajax-bind])',{ ajax: _this },_this.click);
    ...
    ajax.prototype.update = function(event) {
        ...
        var options = $.extend({}, _ajax.options, params);
        var arr = [];
        $(params.tabcont+" input[type=" + name + "]:checked").each(function() {
            arr.push($(this).val());
        })
