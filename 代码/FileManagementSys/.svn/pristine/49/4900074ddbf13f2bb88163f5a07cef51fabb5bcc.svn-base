	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			...
			if(_config.maxSetting !== 'undefined') {
				...
				/*if (currentTabCount >= 2) {
					 layer.msg('为了系统的流畅度，只能同时打开' + 1 + '个选项卡。');
                     return;
	            }*/
				
				//修改时间2018/1/11
				 $("ul.layui-tab-title li").each(function (index, obj) { 
					 var aa=$(this).attr("lay-id");
					 if(aa!=0){
						//删除指定Tab项
					      element.tabDelete(ELEM.tabFilter, aa); 
					 }
		...
		if(_config.contextMenu) {
			element.on('tab(' + ELEM.tabFilter + ')', function(data) {
				 ...
				 $("#theme_id_modify").val($(this).attr('lay-id'));
